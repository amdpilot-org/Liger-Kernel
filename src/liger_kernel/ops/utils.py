...
def calculate_settings(n):
    # gfx950 (MI350) tuned launch config.
    # 64-lane wavefronts: num_warps here == number of 64-thread wavefronts.
    MAX_FUSED_SIZE = 65536
    BLOCK_SIZE = triton.next_power_of_2(n)
    if BLOCK_SIZE > MAX_FUSED_SIZE:
        raise RuntimeError(
            f"Cannot launch: feature dim {n} -> BLOCK_SIZE {BLOCK_SIZE} "
            f"exceeds MAX_FUSED_SIZE {MAX_FUSED_SIZE}."
        )
    num_warps = 4
    if BLOCK_SIZE >= 2048:
        num_warps = 8
